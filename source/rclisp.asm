...
#include   build.inc
           ...
           lbr     f_initcall
#endif
start:     ldi     high stack          ; set stack to top of 32k
           phi     r2
           ldi     low stack
           plo     r2
...
; *****************************************
           mov     r8,oblist2          ; get oblist pointer
           mov     r7,oblist           ; destination
...
zerolp:    glo     re                  ; zero memory
           ...
           str     r2
           ghi     r7
           sm                          ; stop just below end of memory
           bnz     zerolp              ; loop back if not at end
...
mainlp:    ldi     0                   ; set nest level to zero
           ...
           dw      input
           mov     rf,dta              ; convert to uppercase
           sep     scall
           dw      touc
           mov     rf,dta              ; convert to uppercase
...
mainego3:  inc     r7                  ; move past address
           ...
           ldi     high cmderrmsg      ; point to error message
           phi     rf
           ldi     low cmderrmsg
           plo     rf
...
mainego1:  lda     r7                  ; get command byte
           ...
           mov     r8,jump1            ; point to jump address
           ldi     0c0h                ; need LBR
           str     r8                  ; write into jump
           inc     r8
...
sexp:      ldi     0                   ; setup parens count
           ...
           dw      o_msg
           lbr     mainlp
evalerror: ldi     high everrmsg       ; point to error message
           phi     rf
           ldi     low everrmsg
           plo     rf
...
input:     ghi     rc                  ; need to display nest level
           ...
           sep     scall               ; and display it
           dw      o_msg
           mov     rf,dta              ; point to input buffer
...
identatom: ldi     0                   ; indicate an atom
           shr
...
evprim:    mov     rf,jump1            ; point to jump address
           ldi     0c0h                ; need LBR
           str     rf                  ; store into jump
           inc     rf                  ; and increment
           glo     r7                  ; move to binding
           adi     14
           plo     r7
           ghi     r7
...
readbyte:  glo     rf
           ...
           smi     01ah               ; see if past EOF
           lbz     readbno            ; jump if so
#endif
           ldi     0
readbcnt:  shr
           ldi     high buffer
...
welcome:   db      10,13,'Rc/Lisp v 1.1',10,13
           db      'by Michael H. Riley'
crlfmsg:   db      10,13,0
prompt:    db      '>',0
errmsg:    db      'File not found',10,13,0
everrmsg:  db      '??? Evaluation Error ???',10,13,10,13,0
cmderrmsg: db      'Invalid command',10,13,10,13,0
